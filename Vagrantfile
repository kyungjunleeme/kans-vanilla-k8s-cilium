...
#-----Manager Node
    config.vm.define "k8s-s" do |subconfig|
...
#-----Worker Node Subnet1
  (1..N).each do |i|
    config.vm.define "k8s-w#{i}" do |subconfig|
      ...
      subconfig.vm.provision "shell", path: "scripts/k8s-w.sh", args: [ i, k8s_V ]
    end
  end

#-----testpc
    config.vm.define "testpc" do |subconfig|
      subconfig.vm.box = BOX_IMAGE
      subconfig.vm.box_version = BOX_VERSION
      subconfig.vm.provider "vmare_desktop" do |v|
        v.customize ["modifyvm", :id, "--groups", "/#{proj_N}"]
      ...
      subconfig.vm.provision "shell", path: "scripts/testpc.sh", args: [ k8s_V ]
    end
